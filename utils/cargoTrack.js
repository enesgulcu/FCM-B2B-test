...
export const trackShipment = async (cargoKey, custParams = {}) => {
  ...
  if (!invCustId && !senderCustId && !receiverCustId) {
    throw new Error(
      "invCustId, senderCustId veya receiverCustId alanlarından en az biri dolu olmalıdır."
    );
  }
...
<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/"
  ...
  try {
    ...
    console.log("👥 Müşteri Parametreleri:", {
      invCustId,
      senderCustId,
      receiverCustId,
    });
    ...
    if (errorDetail.includes("Endpoint") || errorDetail.includes("ENOTFOUND")) {
      throw new Error(
        "Yurtiçi Kargo servisi şu anda kullanılamıyor. Lütfen daha sonra tekrar deneyin."
      );
    } else if (errorDetail.includes("NumberFormatException")) {
      throw new Error(
        "Müşteri kodu formatı hatalı. Lütfen müşteri kodlarını kontrol edin."
      );
    } else {
      throw error;
    }
